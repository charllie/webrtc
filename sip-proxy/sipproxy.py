...
rx_proxy_authenticate = re.compile("^Proxy-Authenticate:")
rx_proxy_authorization = re.compile("^Proxy-Authorization:")
rx_proxy_auth_username = re.compile(', username="([0-9A-Za-z]*)"')
rx_proxy_auth_realm = re.compile(', realm="([0-9A-Za-z.]*)"')
rx_proxy_auth_uri = re.compile(', uri="([0-9A-Za-z:.]*)"')
rx_proxy_response = re.compile(', response="([0-9A-Za-z]*)"')
rx_proxy_cnonce = re.compile(', cnonce="([0-9A-Za-z]*)"')
rx_proxy_nc = re.compile(', nc=([0-9A-Za-z]*)')
...
class TCPHandler(SocketServer.BaseRequestHandler):   
    ...
    def checkValidity(self,uri):
        addrport, sock, client_addr, validity = registrar[uri]
        now = int(time.time())
        if validity > now:
            return True
        else:
    ...
    def sendResponse(self,code):
        ...
        for line in self.data:
            ...
            if rx_ccontentlength.search(line):
                data[index]="l: 0"
            if rx_proxy_authenticate.search(line):
                data[index]=line
            index += 1
            if line == "":
                break
    ...
    def processRegister(self):
        ...
        for line in self.data:
            ...
            if rx_proxy_authorization.search(line):
                authorization = True
        ...
        registrar[fromm]=[contact,self,self.client_address,validity]
        self.debugRegister()

        #authentication here
        if not authorization:
            self.data.insert(len(self.data)-2, 'Proxy-Authenticate: Digest realm="' + ipaddress + '", qop="auth", nonce="' + NONCE + '", opaque="", stale=FALSE, algorithm=MD5')
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            if registrar.has_key(destination) and self.checkValidity(destination):
                sock,claddr = self.getSocketInfo(destination)
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if registrar.has_key(origin):
                sock,claddr = self.getSocketInfo(origin)
                self.data = self.removeRouteHeader()
                data = self.removeTopVia()
                text = string.join(data,"\r\n")
                #sock.sendto(text,claddr)
                sock.sendall(text)
                showtime()
                logging.info("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text),text))
    ...
    def sendall(self,buffer):
        print buffer
        self.ws.sendData(buffer)
        status = self.ws.status()
        if status == 3:
            response = self.ws.result()
            hexdump(response,' ',16)
    ...
    def handle(self):
        ...
        self.ws = wsserver.wsserver()
        #print ("handle method")
        try:
            rawdata = self.request.recv(8192)
            while len(rawdata):
                hexdump(rawdata,' ',16)
                self.ws.dataRecv(rawdata)
                status = self.ws.status()
                print status
                if status == 0:
                    rawdata = self.request.recv(8192)
